...
def macos_flags() -> tuple[str, dict[str, str]]:
    ...
    if brew_clang:
        return (brew_clang, {"cc-flags": ASAN_COMPILE_FLAGS, "cc-link-flags": "-fsanitize=address", "detect_leaks": "1"})
    ...
    if result.returncode == 0:
        return (system_cc, {"cc-flags": ASAN_COMPILE_FLAGS, "cc-link-flags": "-fsanitize=address", "detect_leaks": "0"})
...
def linux_flags() -> tuple[str, dict[str, str]]:
    ...
    return (cc, {"cc-flags": ASAN_COMPILE_FLAGS, "cc-link-flags": "-fsanitize=address", "detect_leaks": "1"})
...
def patch_link_native_json(
    ...
    is_entry: bool, patch_stubs: bool = True,
) -> None:
    """Patch link.native in a parsed JSON dict with ASan flags.

    When patch_stubs is True (default), patches stub-cc-flags.
    Only patches cc-flags when is_entry is True.
    Always patches cc-link-flags on entry packages.
    """
    ...
    # cc-link-flags: ensure ASan runtime is linked on entry packages.
    # Not all platforms need this (MSVC auto-links the ASan runtime).
    if is_entry and "cc-link-flags" in flags:
        asan_link = flags["cc-link-flags"]
        existing_link_flags = native.get("cc-link-flags", "")
        if existing_link_flags:
            native["cc-link-flags"] = existing_link_flags + " " + asan_link
        else:
            native["cc-link-flags"] = asan_link

    link["native"] = native


def patch_json_file(
    pkg_path: Path, flags: dict[str, str], is_entry: bool,
    patch_stubs: bool = True,
) -> str:
    ...
    try:
        patch_link_native_json(moon_pkg, flags, pkg_path, is_entry, patch_stubs)
...
def _find_object_block(
    ...
    for m in re.finditer(rf"{key_pattern}\s*:\s*\{{", haystack):
        abs_start = search_start + m.start()
        if _is_in_comment(text, abs_start):
            continue
...
def _find_options_block(text: str) -> tuple[int, int] | None:
    ...
    for m in re.finditer(r"\boptions\s*\(", text):
...
def _ensure_native_block(text: str) -> str:
    ...
    # No options() or link: block found — append a new options() block.
    # We must NOT fall back to _find_root_block here because the first { ... }
    # in a moon.pkg file is typically the import block, not a config object.
    return text.rstrip() + '\n\noptions(\n  link: { "native": {} },\n)\n'
...
def patch_dsl_file(
    ...
    # 3. cc-link-flags: ensure ASan runtime is linked on entry packages.
    # Not all platforms need this (MSVC auto-links the ASan runtime).
    if is_entry and "cc-link-flags" in flags:
        asan_link = flags["cc-link-flags"]
        existing = _find_string_value_in_native(text, "cc-link-flags")
        if existing is not None:
            new_value = f"{existing} {asan_link}"
            text = _replace_or_insert_in_native(text, "cc-link-flags", new_value)
        else:
            text = _replace_or_insert_in_native(
                text, "cc-link-flags", asan_link
            )
...
def _is_entry_package(pkg_path: Path) -> bool:
    ...
    if is_dsl_format(pkg_path):
        ...
        for m in pattern.finditer(text):
            if not _is_in_comment(text, m.start()):
                return True
    else:
...
def _discover_entry_packages(repo_root: Path, exclude: set[Path]) -> list[Path]:
    """Auto-discover all entry packages in the repo not already in exclude set.

    Scans for moon.pkg and moon.pkg.json files, checks if they are entry
    packages, and returns those not already in the exclude set. Skips
    _build/ and .git/ directories.
    """
    entry_paths: list[Path] = []
    for pkg_path in sorted(repo_root.rglob("moon.pkg")):
        # Skip build artifacts and git dirs
        parts = pkg_path.relative_to(repo_root).parts
        if any(p.startswith(".") or p == "_build" for p in parts):
            continue
        resolved = pkg_path.resolve()
        if resolved in exclude:
            continue
        if _is_entry_package(resolved):
            entry_paths.append(resolved)
    for pkg_path in sorted(repo_root.rglob("moon.pkg.json")):
        ...
        if resolved in exclude:
            continue
        # Skip if DSL version already found
        dsl_version = resolved.with_name("moon.pkg")
        if dsl_version in exclude or dsl_version in {p for p in entry_paths}:
            continue
        if _is_entry_package(resolved):
            entry_paths.append(resolved)
    return entry_paths
...
def main():
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Must include ALL packages with native-stub. "
            "Entry packages (is-main/test) are auto-discovered for link flag patching."
        ),
    )
    parser.add_argument(
        "--no-disable-mimalloc",
    ...
    # Auto-discover entry packages for link flag patching.
    # cc-link-flags in moon.pkg does NOT propagate to downstream packages,
    # so we must patch every entry package (is-main / has tests) individually.
    extra_entry_paths = _discover_entry_packages(repo_root, seen_pkg_paths)
    if extra_entry_paths:
        print(
            f"Auto-discovered {len(extra_entry_paths)} entry package(s) "
            "for link flag patching"
        )

    # Snapshot originals (both explicit and auto-discovered)
    snapshots: dict[Path, str] = {}
    for pkg_path in pkg_paths:
        snapshots[pkg_path] = pkg_path.read_text(encoding="utf-8")
    for pkg_path in extra_entry_paths:
        snapshots[pkg_path] = pkg_path.read_text(encoding="utf-8")
    ...
    try:
        # Patch explicitly listed packages (full: stub-cc-flags + cc-flags + cc-link-flags)
        for pkg_path in pkg_paths:
        ...
        # Patch auto-discovered entry packages (cc-flags + cc-link-flags only)
        for pkg_path in extra_entry_paths:
            if is_dsl_format(pkg_path):
                patched = patch_dsl_file(
                    pkg_path, flags, is_entry=True, patch_stubs=False
                )
            else:
                patched = patch_json_file(
                    pkg_path, flags, is_entry=True, patch_stubs=False
                )
            pkg_path.write_text(patched, encoding="utf-8")
            fmt = "DSL" if is_dsl_format(pkg_path) else "JSON"
            print(
                f"Patched ({fmt}, entry, auto): "
                f"{display_path(pkg_path, repo_root)}"
            )
